...
- Name: tier1_features.spec.ts >> F5: Skills Grid Layout >> test 25: Architecture card with visual icon exists
- Location: tests\e2e\tier1_features.spec.ts:222:7
...
Locator: locator('[data-testid="skill-card-architecture"], [data-testid="skill-card"]:has-text("Architecture")').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="skill-card-architecture"], [data-testid="skill-card"]:has-text("Architecture")').first()
