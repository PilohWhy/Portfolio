...
- Name: tier2_boundaries.spec.ts >> F4 Edge Cases >> test 20: layout integrity on mobile viewports
- Location: tests\e2e\tier2_boundaries.spec.ts:232:7
...
Locator: locator('[data-testid="reviews-grid"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="reviews-grid"]').first()
...
```ts
  ...
  143 |     
  144 |     // Go forward
  145 |     await page.goForward();
  146 |     await page.waitForURL(/\/projects\/.+/);
  147 |     expect(page.url()).toContain('/projects/');
  148 |   });
  149 | 
